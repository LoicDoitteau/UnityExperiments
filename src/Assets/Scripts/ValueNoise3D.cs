...
public static class ValueNoise3D
{
	...
	public static float Noise(Vector3 point, float frequency, int octaves)
	{
		float value = Value(point, frequency);
		float amplitude = 1f;
		float range = 1f;
		for(int i = 1; i < octaves; i++)
		{
			...
			amplitude *= 0.5f;
			range += amplitude;
			value += Value(point, frequency) * amplitude;
		}
		return  value / range;
	}

	private static float Value(Vector3 point, float frequency)
	{
		point *= frequency;
